...
test.describe("Quiz submissions scores", () => {
  test("Submission of a perfect score should show perfect result and modal", async ({
    page,
  }) => {
  ...
  test("Submission of a 8/10 score should show corresponding result and modal", async ({
    page,
  }) => {
...
test("When reviewing incorrect answers, the correct answer is marked as green and the incorrect answer is marked in red", async ({
  page,
}) => {
  await page.getByText("Germany").first().click();
  await page.getByText("9", { exact: true }).click();
  ...
  await expect(page.getByText("9", { exact: true })).toHaveCSS(
    "background-color",
    "rgb(255, 134, 128)",
  );
  await expect(page.getByText("France")).toHaveCSS(
    "background-color",
    "rgb(200, 247, 197)",
  );
  await expect(page.getByText("11")).toHaveCSS(
    "background-color",
    "rgb(200, 247, 197)",
  );
});
